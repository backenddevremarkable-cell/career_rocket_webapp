...
function idealCareerTest() {

  const { users, ptInfo, setPtinfo } = useDataStore((state) => state);
  const [info, setInfo] = useState(ptInfo);
  ...
  const getData = async () => {
    try {
      ...
      const response = await axios.post(`${RE_API}pt-test-info`, updatePayload)
          ...
          <div className="grid grid-cols-1 gap-3 lg:grid-cols-12">
            ...
            <div className="lg:col-span-4">
              <div className="sticky rounded-[12px] border border-gray-200 bg-white p-5 shadow-xl shadow-gray-200/70">
                ...
                <Link href={'/attempt-test/psychometric-test'} className="mt-7 flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-fuchsia-700 to-purple-600 px-6 py-4 text-lg font-semibold text-white transition hover:opacity-95">
...
export default withAuth(idealCareerTest);