...
export default function MomentsSlider() {
  ...
  return (data && data?.length > 0 || loading ?
            ...
            >
              {data?.map((data, i) => (
                <SwiperSlide key={i}>
                  {({ isActive }) => (
                    <div
                      className={`transition-all duration-500 rounded-2xl overflow-hidden cursor-pointer ${isActive ? "scale-130 z-10" : "scale-70 opacity-70"
                        }`}
                    >
                      <img
