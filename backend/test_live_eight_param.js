...
async function run() {
    ...
    // Common 8-parameter orderings
    const orderings = [
        // 1. MerchantCode + ServiceID + RequestID + RevenueHead + ConsumerKey + ConsumerName + SsoID + SsoToken
        ['MERCHANTCODE', 'SERVICEID', 'REQUESTID', 'REVENUEHEAD', 'CONSUMERKEY', 'CONSUMERNAME', 'SSOID', 'SSOTOKEN'],
        // 2. MerchantCode + RequestID + ServiceID + RevenueHead + ConsumerKey + ConsumerName + SsoID + SsoToken
        ['MERCHANTCODE', 'REQUESTID', 'SERVICEID', 'REVENUEHEAD', 'CONSUMERKEY', 'CONSUMERNAME', 'SSOID', 'SSOTOKEN'],
        // 3. Alphabetical order of 8 parameters
        ['CONSUMERKEY', 'CONSUMERNAME', 'MERCHANTCODE', 'REQUESTID', 'REVENUEHEAD', 'SERVICEID', 'SSOID', 'SSOTOKEN'],
        // 4. Insertion order of 8 parameters
        ['MERCHANTCODE', 'REQUESTID', 'SERVICEID', 'REVENUEHEAD', 'CONSUMERKEY', 'CONSUMERNAME', 'SSOID', 'SSOTOKEN'],
        // 5. MerchantCode + ServiceID + RequestID + ReqTimestamp + RevenueHead + ConsumerKey + ConsumerName + SsoToken
        ['MERCHANTCODE', 'SERVICEID', 'REQUESTID', 'REQTIMESTAMP', 'REVENUEHEAD', 'CONSUMERKEY', 'CONSUMERNAME', 'SSOTOKEN'],
        // 6. Alphabetical order with ReqTimestamp instead of SsoID
        ['CONSUMERKEY', 'CONSUMERNAME', 'MERCHANTCODE', 'REQUESTID', 'REQTIMESTAMP', 'REVENUEHEAD', 'SERVICEID', 'SSOTOKEN']
    ];
    ...
    console.log(`Starting live execution for 8-param variants...`);
        ...
        for (const keyCand of keyCandidates) {
            ...
            for (const nameVal of nameVariants) {
                ...
                for (const f of formulas) {
                    ...
                    if (index % 50 === 0) {
                        console.log(`Tested ${index} 8-param live permutations...`);
                    }
    ...
    console.log("Completed. No 8-param formula succeeded.");
}
