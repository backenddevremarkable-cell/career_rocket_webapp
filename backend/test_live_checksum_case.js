...
async function run() {
    ...
    const ssoidVariants = ["PRACHEE.GAUR", "prachee.gaur"];
    const keyCandidates = [
        { label: "No Key", val: "" },
    ...
    console.log(`Starting live execution for case variants...`);

    let index = 0;
    for (const SSOID of ssoidVariants) {
        const paramValues = {
            SSOID,
            REQUESTID,
            REQTIMESTAMP,
            ...
            for (const keyCand of keyCandidates) {
                ...
                for (const f of formulas) {
                    ...
                    const payload = {
                        ...
                        CONSUMERNAME,
                        COMMTYPE,
                        SSOID, // Matching current ssoid case
                        OFFICECODE,
                        SSOTOKEN,
                        CHECKSUM: f.checksum
                    };
                    ...
                    if (index % 100 === 0) {
                        console.log(`Tested ${index} case-variant live permutations...`);
                    }
    ...
    console.log("Completed. No case-variant formula succeeded.");
}
